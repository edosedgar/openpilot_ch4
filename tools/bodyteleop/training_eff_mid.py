...
transform = transforms.Compose([
    transforms.Resize((3, 480)), # Adjust based on the exact variant
    transforms.ToTensor(),
...
for epoch in range(num_epochs):
    ...
    if (epoch + 1) % N == 0 or val_acc > best_val_acc:
        ...
        checkpoint_name = os.path.join(args.output_dir, f'mid-efficient-checkpoint_epoch{epoch+1}_valacc{val_acc:.4f}.pth')
        checkpoint = {
            'epoch': epoch + 1,
...
torch.save(model.state_dict(), "self_driving_rc_model_efficient.pth")
model.load_state_dict(torch.load("self_driving_rc_model_efficient.pth"))
